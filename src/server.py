...
class Server:
    ...
    def __init__(self, ip, port, exit_signal):
...
class ProtocolHandler:
    ...
    def cli_login(player, protocol):
        ...
        # 发送登录成功协议
        player.send(
            {"protocol": "ser_login", "result": True, "player_data": player.game_data}
        )

        # 发送上线信息给其他玩家
        player.send_without_self(
            {"protocol": "ser_online", "player_data": player.game_data}
        )
    ...
    def cli_move(player, protocol):
        ...
        player.game_data["t"] = protocol.get("t")

        # 告诉所有玩家当前玩家坦克的位置发生变化
        player.send_all_player(
            {"protocol": "ser_move", "player_data": player.game_data}
        )

    @staticmethod
    def player_pos(player, protocol):
        """
        实时更新客户端玩家坦克位置
        """
        # 如果玩家未登录，则不做处理
        ...
        # 实时更新客户端玩家坦克的位置
        player.game_data["dir"] = protocol.get("dir")
        player.game_data["left"] = protocol.get("left")
        player.game_data["top"] = protocol.get("top")

        # 告诉其他玩家当前玩家坦克的位置发生变化
        player.send_without_self(
            {"protocol": "player_pos", "player_data": player.game_data}
        )
    ...
    def game_start(player, protocol):
        ...
        player.send_all_player(
            {"protocol": "game_start", "player_data": player.game_data}
        )
    ...
    def game_over(player, protocol):
        ...
        player.send_all_player(
            {"protocol": "game_over", "player_data": player.game_data}
        )
    ...
    def player_shoot(player, protocol):
        ...
        player.send_all_player(
            {"protocol": "player_shoot", "player_data": player.game_data}
        )

    @staticmethod
    def player_injured(player, protocol):
        """
        通知所有客户端玩家该坦克受到子弹攻击
        """
        # 如果玩家未登录，则不做处理
        ...
        player.send_all_player(
            {
                "protocol": "player_injured",
                "role_id": protocol.get("role_id"),
                "player_data": player.game_data,
            }
        )

    @staticmethod
    def player_die(player, protocol):
        """
        通知所有客户端玩家该坦克死亡
        """
        # 如果玩家未登录，则不做处理
    ...
    def enemy_shoot(player, protocol):
        ...
        # 告诉所有玩家敌人坦克发射出子弹
        player.send_all_player(
            {"protocol": "enemy_shoot", "player_data": player.game_data}
        )
    ...
    def enemy_birth(player, protocol):
        ...
        player.game_data["tank_id"] = protocol.get("tank_id")

        # 告诉所有玩家有新的敌人出现
        player.send_all_player(
            {"protocol": "enemy_birth", "player_data": player.game_data}
        )
    ...
    def enemy_move(player, protocol):
        ...
        # 告诉所有玩家敌人的坦克发生了移动
        player.send_all_player(
            {"protocol": "enemy_move", "player_data": player.game_data}
        )

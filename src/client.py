...
class Client:
    ...
    def protocol_handler(self, protocol):
        ...
        if protocol["protocol"] == "ser_login":
            ...
            role_id = protocol["player_data"]["role_id"]
            all_tank_group = net_tank_world.Net_tank_world.globalV["all_tank_group"]
            back_ground = net_tank_world.Net_tank_world.globalV["back_ground"]

            frame = protocol["player_data"]["t"]
            if (
                frame
                > net_tank_world.Net_tank_world.player_tanks_list[role_id - 1].frame
            ):
                net_tank_world.Net_tank_world.player_tanks_list[
                    role_id - 1
                ].frame = frame
                net_tank_world.Net_tank_world.player_tanks_list[role_id - 1].moving1 = 7
                net_tank_world.Net_tank_world.player_tanks_list[
                    role_id - 1
                ].direction = protocol["player_data"]["dir"]
                all_tank_group.remove(
                    net_tank_world.Net_tank_world.player_tanks_list[role_id - 1]
                )
                if net_tank_world.Net_tank_world.player_tanks_list[
                    role_id - 1
                ].move_func(
            ...
            net_tank_world.Net_tank_world.player_tanks_list[
                role_id - 1
            ].rect.left = protocol["player_data"]["left"]
            net_tank_world.Net_tank_world.player_tanks_list[
                role_id - 1
            ].rect.top = protocol["player_data"]["top"]

        elif protocol["protocol"] == "player_shoot":
            role_id = protocol["player_data"]["role_id"]
            net_tank_world.Net_tank_world.player_tanks_list[role_id - 1].shoot()
            net_tank_world.Net_tank_world.player_tanks_list[
                role_id - 1
            ].bullet_not_cooling = True
        elif protocol["protocol"] == "player_injured":
            role_id = protocol["role_id"]
            net_tank_world.Net_tank_world.player_tanks_list[role_id - 1].life -= 1
        elif protocol["protocol"] == "player_die":
            role_id = protocol["role_id"]
            net_tank_world.Net_tank_world.player_tanks_list[role_id - 1].life = 0
        elif protocol["protocol"] == "enemy_birth":
            all_tank_group = net_tank_world.Net_tank_world.globalV["all_tank_group"]
            enemy_tank_group = net_tank_world.Net_tank_world.globalV["enemy_tank_group"]
            ...
            if "enemy_tank_group" in net_tank_world.Net_tank_world.globalV:
                enemy_tank_group = net_tank_world.Net_tank_world.globalV[
                    "enemy_tank_group"
                ]
                ...
                for enemy_tank in enemy_tank_group:
                    ...
                    if tank_id in info:
                        enemy_tank.move_sync(
                            info[tank_id][0], info[tank_id][1], info[tank_id][2]
                        )
                    else:
            ...
            if "enemy_tank_group" in net_tank_world.Net_tank_world.globalV:
                ...
                enemy_bullet_group = net_tank_world.Net_tank_world.globalV[
                    "enemy_bullet_group"
                ]
                for enemy_tank in enemy_tank_group:
    ...
    def login(self, username, password):
        ...
        登录
        """
        data = {"protocol": "cli_login", "username": username, "password": password}
        self.send(data)
    ...
    def move(self, player):
        ...
        玩家移动
        """
        data = {"protocol": "cli_move", "x": player.next_mx, "y": player.next_my}
        self.send(data)

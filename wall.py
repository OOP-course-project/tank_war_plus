...
class Map:
    def __init__(self, map_file):
        ...
        for brick_data in map_data['bricks']:
            self.brick = Brick()
            self.brick.rect.left, self.brick.rect.top = 3 + brick_data['x'] * 24, 3 + brick_data['y'] * 24
            self.brick_group.add(self.brick)

        # 用加载的数据创建铁块
        for iron_data in map_data['irons']:
            self.iron = Iron()
            self.iron.rect.left, self.iron.rect.top = 3 + iron_data['x'] * 24, 3 + iron_data['y'] * 24
            self.iron_group.add(self.iron)

# my_map = Map(r"../map/level1.json")
